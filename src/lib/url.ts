 ...
 * Base-path helpers.
 *
 * The site may be deployed under a sub-path (`base` in astro.config.mjs), so no
 * internal link, asset reference or absolute URL may be written as a root-absolute
 * `/foo/` string. Everything goes through these helpers.
 *
...
/** Astro injects the configured base here — "/blog/" under a sub-path, "/" at root. */
